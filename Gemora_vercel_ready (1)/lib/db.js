...
async function seed(){
  ...
  await query(`INSERT INTO users (email,password,role) VALUES ($1,$2,'ADMIN') ON CONFLICT (email) DO NOTHING`, [adminEmail, adminPass]);
  const sellerEmail='seller@gemora.test', sellerPass='password';
  await query(`INSERT INTO users (email,password,role) VALUES ($1,$2,'USER') ON CONFLICT (email) DO NOTHING`, [sellerEmail, sellerPass]);
  const s = await query('SELECT id FROM users WHERE email=$1',[sellerEmail]); const sellerId = s.rows[0].id;
  const c = await query('SELECT COUNT(*) FROM listings'); if (parseInt(c.rows[0].count)===0){
    const items=[
    ...
    for(const i of items){ await query('INSERT INTO listings (title,description,price_cents,image_url,seller_id,status) VALUES ($1,$2,$3,$4,$5,'ACTIVE')',[i.t,i.d,i.p,i.u,sellerId]); }
  }
  // Seed a couple of pending orders to populate admin dashboard
  const l = await query('SELECT id, price_cents FROM listings ORDER BY id ASC LIMIT 2');
  for (const row of l.rows){
    await query(`INSERT INTO orders (listing_id,buyer_email,amount_cents,paystack_ref,status) VALUES ($1,$2,$3,$4,$5)`,
      [row.id, 'buyer@example.com', row.price_cents, 'TESTREF-'+row.id, 'PENDING']);
  }
}
let initialized=false; async function ensureReady(){ if(initialized) return; await migrate(); await seed(); initialized=true; }
